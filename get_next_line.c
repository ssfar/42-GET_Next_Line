...
/*   Updated: 2018/12/03 02:56:49 by ssfar            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

t_glist		*ft_glstnew(char *s, int fd, t_glist *first)
{
	t_glist	*list;
...
int			get_line(t_glist **first, t_glist *lst, char **line)
{
	t_glist *prev;
...
int			fill_lst(t_glist **first, t_glist **lst, int fd, char *buf)
{
	int		ret;
	char	*to_free;
	...
	while ((ret = read(fd, buf, BUFF_SIZE)) > 0)
	{
		...
		if (!*lst)
		{
			if (!(*lst = ft_glstnew(ft_strdup(buf), fd, *first)))
				return (0);
			*first = *lst;
		}
		else if ((to_free = (*lst)->s))
		{
